...
def test_transaction_update_exceeds_balance():
    with pytest.raises(Exception, match='Amount exceeds balance'):
        Transaction(Wallet(), 'recipient', 199020)
...
def test_valid_transation():
    Transaction.is_valid_transaction(Transaction(Wallet(), 'recipient', 50))

def test_valid_transation_with_invalid_outputs():
    sender_wallet = Wallet()
...
def test_valid_transation_with_invalid_signature():
...
def test_invalid_reward_transaction_invalid_amount():
    ...
    reward_transaction.output[miner_wallet.address] = 90000
